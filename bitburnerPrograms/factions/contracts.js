  ...
  while (hostsToScan.length > 0 && infiniteLoopProtection-- > 0) { // Loop until the list of hosts to scan is empty
    ...
    for (const fileName of files) {
      if (fileName.includes(".cct")) {
        ...
        contractprint(ns,fileName,contractServersFound,searchContractsFound);
      }
    }
  }
...
function contractprint(ns,fileName, contractServersFound,searchContractsFound) {
  if (searchContractsFound > 0) {
    ns.tprint("-------------------------------------------------------------");
    ns.tprint(searchContractsFound + " contracts found for " + fileName + " !!!!");
    ns.tprint("On servers: " + contractServersFound);
  }
  else {
    ns.tprint("No contracts found for " + fileName + ".");
  }
}